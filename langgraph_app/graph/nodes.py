...
def make_answer_evaluator(llm, node_name: str = "answer_evaluator"):
    ...
    def _build_semantic_concept_key(state: RAGState) -> str:
        question = (state.get("question") or "").lower()
        hint = (state.get("check_answer_hint") or "").lower()
        combined = f"{question} {hint}"
    ...
    def _build_concept_trace(state: RAGState) -> dict:
        docs = state.get("docs", []) or []
        if docs:
            top_doc = docs[0]
    ...
    def answer_evaluator(state: RAGState) -> RAGState:
        ...
        student_db = state.get("student_db")
        student_id = state.get("student_id")
        concept_key = _build_semantic_concept_key(state)
        concept_trace = _build_concept_trace(state)
        mastery_event = None
